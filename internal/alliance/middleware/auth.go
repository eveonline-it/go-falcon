...
// AuthMiddleware provides authentication and authorization for alliance
...
// RequireAdminAccess ensures the user has administrative access (super admin)
func (m *AuthMiddleware) RequireAdminAccess(ctx context.Context, authHeader, cookieHeader string) (*models.AuthenticatedUser, error) {
	...
	if m.permissionManager != nil {
		hasPermission, err := m.permissionManager.HasPermission(ctx, int64(user.CharacterID), "system:admin:full")
	...
	return nil, huma.Error403Forbidden("Insufficient permissions: administrative access required")
}
...
func (m *AuthMiddleware) RequirePermission(ctx context.Context, authHeader, cookieHeader, permissionID string) (*models.AuthenticatedUser, error) {
	...
	if m.permissionManager != nil {
		...
		if !hasPermission {
			return nil, huma.Error403Forbidden("Permission denied: " + permissionID + " required")
		}
