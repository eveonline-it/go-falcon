...
func (m *MockUserCharacterResolver) GetUserWithCharacters(ctx context.Context, userID string) (*middleware.UserWithCharacters, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*middleware.UserWithCharacters), args.Error(1)
}

// TestData contains common test data
type TestData struct {
	TestUser *models.AuthenticatedUser
	TestUserWithCharacters *middleware.UserWithCharacters
}
...
func TestEnhancedAuthMiddleware_Integration(t *testing.T) {
	...
	t.Run("CharacterResolutionMiddleware", func(t *testing.T) {
		...
		// Configure mock expectations
		mockCharacterResolver.On("Getmiddleware.UserWithCharacters", 
			mock.Anything, testData.TestUser.UserID).Return(testData.Testmiddleware.UserWithCharacters, nil)
