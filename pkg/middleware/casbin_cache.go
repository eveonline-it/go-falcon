...
func (s *CachedCasbinService) getUserExpandedContext(ctx context.Context, userID string) (*ExpandedAuthContext, error) {
	...
	cached, err := s.redisClient.Get(ctx, cacheKey).Result()
	if err == nil {
		var expandedCtx ExpandedAuthContext
...
func (s *CachedCasbinService) SyncUserHierarchy(ctx context.Context, userID string, characters []UserCharacter) error {
	// Sync the hierarchy
...
func (c *CachedCasbinAuthMiddleware) checkHierarchicalPermission(ctx context.Context, authCtx *ExpandedAuthContext, resource, action string) (bool, error) {
