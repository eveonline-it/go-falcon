...
1. **Development Server (Hot Reload)**
   ```bash
   # The falcon server runs with hot reload in development
   # No need to manually build - changes are automatically reloaded
   docker-compose up -d  # Starts all infrastructure and falcon with hot reload
   
   # Make code changes and they will be automatically recompiled and restarted
   # Watch the logs: docker-compose logs -f falcon
   ```

2. **Feature Development**
   ```bash
   git checkout -b feature/your-feature
   # Make changes - falcon automatically reloads
   go test ./...
...
- ❌ Never run falcon directly (use Docker with hot reload)
- ❌ Never build manually during development (hot reload handles this)
